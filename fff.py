...
class DiscordLinkExtractor:
    ...
    def save_credentials(self, email_data: Dict):
        try:
            with open("accounts.txt", "a", encoding="utf-8") as f:
                ...
                if email_data.get('refresh_token'):
                    f.write(f"Refresh Token: {email_data['refresh_token']}\n")
                
                if email_data.get('client_id'):
                    f.write(f"Client ID: {email_data['client_id']}\n")
                
                f.write(f"Type: {email_data.get('type', 'Unknown')}\n")
                f.write(f"Price: ${email_data.get('price', 0)}\n")
                f.write("-" * 50 + "\n\n")
            
            print(f"Credentials saved to accounts.txt")
                        ...
                        if 'Accounts' in purchase_data and isinstance(purchase_data['Accounts'], list):
                            ...
                            if len(accounts) > 0:
                                ...
                                if email_:
                                    ...
                                    self.save_credentials(email_data)
                                    return email_data
...
def main():
    ...
    print("=" * 50)
    print("Modified Version - Gets real verification link")
    print("=" * 50)
